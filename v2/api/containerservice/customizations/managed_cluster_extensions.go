...
func (ext *ManagedClusterExtension) ExportKubernetesSecrets(
	...
	if requestedSecrets.Contains(adminCredentials) {
	...
	if requestedSecrets.Contains(userCredentials) {
	...
	if adminCredentials != "" {
		resolvedSecrets[adminCredentials] = adminCredentials
	}
	if userCredentials != "" {
		resolvedSecrets[userCredentials] = userCredentials
	}
...
func secretsSpecified(obj *containerservice.ManagedCluster) set.Set[string] {
	...
	if secrets.AdminCredentials != nil {
		result.Add(adminCredentials)
	}
	if secrets.UserCredentials != nil {
		result.Add(userCredentials)
	}

...
func (r *azureDeploymentReconcilerInstance) saveAssociatedKubernetesResources(ctx context.Context) error {
	originalVersion := r.ObjAsOriginalVersion()

	exporters := []kubernetesResourceExporter{
		&handcraftedKubernetesResourceExporter{
			obj:        r.Obj,
			connection: r.ARMConnection,
			log:        r.Log,
			extension:  r.Extension,
		},
		&autogeneratedKubernetesResourceExporter{
			obj:          r.Obj,
...
// ObjAsOriginalVersion returns r.Obj as the original API version used to create the resource.
func (r *azureDeploymentReconcilerInstance) ObjAsOriginalVersion() genruntime.ARMMetaObject {
	resource := r.Obj

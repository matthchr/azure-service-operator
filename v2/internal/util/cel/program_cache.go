...
type ProgramCache struct {
	...
	cache    *ttlcache.Cache[string, *CompilationResult]
	envCache *EnvCache
...
func NewProgramCache(
	...
	cache := ttlcache.New[string, *CompilationResult](
		ttlcache.WithTTL[string, *CompilationResult](24 * time.Hour),
	)
	cache.OnInsertion(
		func(ctx context.Context, item *ttlcache.Item[string, *CompilationResult]) {
			log.V(Debug).Info("Program cache item inserted", "key", item.Key(), "expiry", item.ExpiresAt())
		})
	cache.OnEviction(
		func(ctx context.Context, reason ttlcache.EvictionReason, item *ttlcache.Item[string, *CompilationResult]) {

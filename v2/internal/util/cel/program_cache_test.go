...
func Test_ProgramCache(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			envCache := asocel.NewEnvCache(newEnvWrapper)

			compileWrapper := func(env *cel.Env, expression string) (*asocel.ProgramCacheItem, error) {
				compileCalled++
				return simpleCompile(env, expression)
			}
			programCache := asocel.NewProgramCache(envCache, compileWrapper)
...
// TODO: We may end up wanting to cache errors too
func Test_ProgramCache_ErrorsAreNotCached(t *testing.T) {
	t.Parallel()
	g := NewGomegaWithT(t)
	...
	g.Expect(compileCalled).To(Equal(10))
}

// This is a simple compilation helper specifically for testing the cache
func simpleCompile(env *cel.Env, expression string) (*asocel.ProgramCacheItem, error) {
	...
	return &asocel.ProgramCacheItem{
		AST:     ast,
		Program: program,
	}, nil
}
